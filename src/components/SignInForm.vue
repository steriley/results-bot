...
<template>
  <div
    class="max-w-md w-full mx-auto p-8 bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-2xl shadow-xl"
  >
    <h2 class="text-2xl font-bold text-slate-900 dark:text-white mb-6 text-center">Welcome Back</h2>

    <div
      v-if="errorMsg"
      class="mb-6 p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/50 rounded-xl text-red-600 dark:text-red-500 text-sm font-medium"
    >
      {{ errorMsg }}
    </div>
    ...
    <form @submit.prevent="handleSignIn" class="space-y-5">
      ...
      >
        <span
          v-if="loading"
          class="mr-2 inline-block h-4 w-4 rounded-full border-2 border-white/20 border-t-white animate-spin"
        ></span>
